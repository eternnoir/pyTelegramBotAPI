...
from telebot import apihelper, util, types, asyncio_helper
...
class AsyncTeleBot:
    ...
    def __init__(self, token: str, parse_mode: Optional[str]=None, offset=None,
        ...
        self.token = token
        logger.info('creating some objects..')
    ...
    async def register_for_reply_by_message_id(
            self, message_id: int, callback: Callable, *args, **kwargs) -> None:
        """
        Registers a callback function to be notified when a reply to `message` arrives.
    ...
    async def _notify_reply_handlers(self, new_messages) -> None:
        """
        Notify handlers of the answers
        :param new_messages:
        :return:
