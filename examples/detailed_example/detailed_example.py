...
imageSelect = ReplyKeyboardMarkup(
...
# user can choose an image (multi-stage command example)
@bot.message_handler(commands=["getImage"])
def command_image(m):
    cid = m.chat.id
    bot.send_message(
        cid, "Please choose your image now", reply_markup=imageSelect
    )  # show the keyboard
    userStep[cid] = (
        1  # set the user to the next step (expecting a reply in the listener now)
    )
...
def msg_image_select(m):
    ...
    if (
        ...
        bot.send_photo(
            cid, open("rooster.jpg", "rb"), reply_markup=hideBoard
