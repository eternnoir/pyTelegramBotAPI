...
def back_keyboard():
    return InlineKeyboardMarkup(
        keyboard=[[InlineKeyboardButton(text="⬅", callback_data="back")]]
    )
...
async def products_command_handler(message: Message):
    await bot.send_message(
        message.chat.id, "Products:", reply_markup=products_keyboard()
    )


# Only product with field - product_id = 2
@bot.callback_query_handler(func=None, config=products_factory.filter(product_id="2"))
async def product_one_callback(call: CallbackQuery):
    await bot.answer_callback_query(
        callback_query_id=call.id, text="Not available :(", show_alert=True
    )


# Any other products
@bot.callback_query_handler(func=None, config=products_factory.filter())
async def products_callback(call: CallbackQuery):
