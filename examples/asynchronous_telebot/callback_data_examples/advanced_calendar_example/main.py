...
async def calendar_command_handler(message: Message):
    now = date.today()
    await bot.send_message(
        message.chat.id,
        "Calendar",
...
async def calendar_zoom_out_handler(call: CallbackQuery):
...
async def callback_empty_field_handler(call: CallbackQuery):
