...
class TestTelegramApiInterface:
    ...
    def test_get_updates(self):
        ...
        api.assert_called_with('getUpdates', params=kwargs)

    def test_get_me(self):
        api = self.create_mocked_instance()
        api.get_me()
        api.assert_called_with('getMe', return_type=types.User)

    def test_get_file(self):
        api = self.create_mocked_instance()
        api.get_file(100)
        api.assert_called_with('getFile', params={'file_id': 100}, return_type=types.File)
    ...
    def test_kick_chat_member(self):
        ...
        api.assert_called_with('kickChatMember', params={'chat_id': 1, 'user_id': 2})

    def test_unban_chat_member(self):
        api = self.create_mocked_instance()
        api.unban_chat_member(1, 2)
        api.assert_called_with('unbanChatMember', params={'chat_id': 1, 'user_id': 2})

    def test_edit_message_text(self):
        api = self.create_mocked_instance()
        api.make_json_request.return_value = True
        api.edit_message_text('new_text', optional=True)
        api.assert_called_with('editMessageText', params={'text': 'new_text', 'optional': True})

    def test_answer_callback_query(self):
        api = self.create_mocked_instance()
        api.answer_callback_query(1, optional=True)
        api.assert_called_with('answerCallbackQuery', params={'callback_query_id': 1, 'optional': True})
    ...
    def create_mocked_instance(self):
        ...
        instance.assert_called_with = instance.make_json_request.assert_called_with
        return instance
